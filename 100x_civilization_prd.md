...
A cohort 2 alumna finished her program a year ago. She has frontend skills and occasional availability. She joins through Google OAuth, completes onboarding, tags 6 skills, and sets availability to "open to paid projects." She gets notified of two relevant opportunities in week 1 without posting anything herself. She replies to one. A connection is made.
...
4. New alumni joins through Google OAuth, completes skill-tag onboarding (minimum 5 tags), waits for admin approval, and immediately sees 3 relevant open opportunities on the feed.
...
1. User visits landing page → clicks Join → Google OAuth
2. Submits onboarding form
3. Receives pending email
4. Admin approves
5. User receives welcome email
6. User accesses feed
...
|---|---|---|---|
| Auth | Google OAuth | As a new user, I sign in with Google so I don't manage a password. Any user can sign up. Access is gated by admin approval. | Landing page, OAuth redirect |
| Admin Approval | Manual review | As an admin, I manually review `pending_users`, approve or reject submissions, and send the appropriate approval email. Approved users get feed access. | Admin panel |
...
| Admin | Review Queue | Admin (Pranamya / team) reviews pending users, approves or rejects submissions, and manages access. | Admin panel |
...
- **User profile fields:** `phone` and `linkedinUrl` are stored on approved user records. The LinkedIn URL is used to enrich embedding input, not to fetch LinkedIn pages or call any LinkedIn API.
- **Post embeddings:** generated at post creation from post type + title + description + skill tags. Stored with the post record.
...
- **Privacy:** no PII beyond name, cohort, email, role, phone, and LinkedIn URL. Contact reveal is gated by user consent toggle.
...
- Test approval gating: unauthenticated user cannot access any page beyond the landing screen, and approved users only reach the feed after manual review.
...
| Unapproved signup attempts | Manual admin approval prevents feed access until review is complete. |
...
1. Google OAuth is the only auth method. Any user can sign up, but admin approval is required before platform access is granted.
...
6. Admin manually approves each user before platform access is granted.
7. All 5 opportunity types share one database schema with a type enum field and conditional required fields per type.
8. No payments, no escrow, no revenue mechanism in v1.
9. Connection is defined as: user A from cohort X views a post by user B from cohort Y AND sends a message or reveals contact info. View alone does not count.
10. pgvector is available on Supabase free tier. If not, cosine similarity computed in application layer on retrieval.
11. **UNVERIFIED:** Post-capstone IP rights permit Sahajta to continue operating and developing the product. Verify before week 2 acquisition begins.
...
- Admin-approval gating limits data collection to reviewed community members
...
| Day | Milestone |
|---|---|
| Day 1 | Supabase schema, Google OAuth, onboarding form, pending approval queue |
...
| Day 5 | Connection logging, cross-cohort pair tracking, admin approval queue |
| Day 6 | QA: 10 synthetic profiles, 5 synthetic posts, match verification |
| Day 7 | Deploy to production. Approve 5 trusted alumni for soft-launch pressure test. Fix critical bugs only. |
